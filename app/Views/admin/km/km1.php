...
<form action="/admin/spt/<?= $spt['id'] ?>/km/1/save" method="POST">
    ...
    <div class="card">
        ...
        <tr>
            ...
            <td style="padding:10px 12px;border:1px solid #e2e8f0">
                <textarea name="kegiatan" class="form-control" rows="4"
                data-wysiwyg data-wysiwyg-height="160px"
            ...
            <td style="padding:10px 12px;border:1px solid #e2e8f0">
                <div style="display:flex;gap:8px;flex-wrap:wrap">
                    <div style="flex:1;min-width:130px">
                        <label style="font-size:11px;color:#64748b;display:block;margin-bottom:3px">Rencana Mulai Pengawasan</label>
                    ...
                    <div style="flex:1;min-width:130px">
                        <label style="font-size:11px;color:#64748b;display:block;margin-bottom:3px">Rencana Penerbitan Laporan</label>
        ...
        <tr>
            ...
            <td style="padding:10px 12px;border:1px solid #e2e8f0">
                <?php
                $bulanOpts7   = ['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'];
                $mingguOpts7  = ['Mg-I','Mg-II','Mg-III','Mg-IV'];
                $tahunOpts7   = range(date('Y') - 1, date('Y') + 2);
                $curKunjungan = old('rencana_kunjungan', $row['rencana_kunjungan'] ?? '');
                $kParts = ['mg'=>'','bln'=>'','thn'=>''];
                if ($curKunjungan && preg_match('/^(Mg-(?:I{1,3}|IV))\s+(\w+)\s+(\d{4})$/', $curKunjungan, $km)) {
                    $kParts = ['mg'=>$km[1],'bln'=>$km[2],'thn'=>$km[3]];
                }
                ?>
                <input type="hidden" name="rencana_kunjungan" id="hid-kunjungan"
                       value="<?= esc($curKunjungan) ?>">
                <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                    <select id="sel-kunjungan-mg" class="form-control form-control-sm no-select2"
                            style="width:90px" onchange="updateKunjungan()" <?= !$canEdit ? 'disabled' : '' ?>>
                        <option value="">Minggu</option>
                        <?php foreach($mingguOpts7 as $mg): ?>
                        <option value="<?= $mg ?>" <?= $kParts['mg']===$mg?'selected':'' ?>><?= $mg ?></option>
                        <?php endforeach; ?>
                    </select>
                    <select id="sel-kunjungan-bln" class="form-control form-control-sm"
                            style="flex:1;min-width:80px" onchange="updateKunjungan()" <?= !$canEdit ? 'disabled' : '' ?>>
                        <option value="">Bulan</option>
                        <?php foreach($bulanOpts7 as $bln): ?>
                        <option value="<?= $bln ?>" <?= $kParts['bln']===$bln?'selected':'' ?>><?= $bln ?></option>
                        <?php endforeach; ?>
                    </select>
                    <select id="sel-kunjungan-thn" class="form-control form-control-sm no-select2"
                            style="width:82px" onchange="updateKunjungan()" <?= !$canEdit ? 'disabled' : '' ?>>
                        <option value="">Tahun</option>
                        <?php foreach($tahunOpts7 as $y): ?>
                        <option value="<?= $y ?>" <?= $kParts['thn']==$y?'selected':'' ?>><?= $y ?></option>
                        <?php endforeach; ?>
                    </select>
                    <div id="preview-kunjungan" style="font-size:12px;color:#6366f1;font-weight:600">
                        <?= $curKunjungan ? '<i class="fas fa-calendar-check"></i> <strong>'.esc($curKunjungan).'</strong>' : '' ?>
                    </div>
                </div>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td style="padding:10px 12px;border:1px solid #e2e8f0">
                <?php
                // Prioritas: nilai tersimpan > prefill dari PKPT > kosong
                $rmpVal = old('rmp_bulan', $row['rmp_bulan'] ?? $pkptRmpBulan ?? '');
                $rplVal = old('rpl_bulan', $row['rpl_bulan'] ?? $pkptRplBulan ?? '');
                ?>
                <div style="display:flex;gap:16px;flex-wrap:wrap">
                    <div>
                        ...
                        <input type="number" name="rmp_bulan" class="form-control form-control-sm"
                               ...
                               value="<?= esc($rmpVal) ?>"
                               <?= !$canEdit ? 'disabled' : '' ?>>
                        <?php if(empty($row['rmp_bulan']) && !empty($pkptRmpBulan)): ?>
                        <small style="font-size:10px;color:#6366f1"><i class="fas fa-bolt"></i> dari PKPT</small>
                        <?php endif; ?>
                    </div>
                    <div>
                        ...
                        <input type="number" name="rpl_bulan" class="form-control form-control-sm"
                               ...
                               value="<?= esc($rplVal) ?>"
                               <?= !$canEdit ? 'disabled' : '' ?>>
                        <?php if(empty($row['rpl_bulan']) && !empty($pkptRplBulan)): ?>
                        <small style="font-size:10px;color:#6366f1"><i class="fas fa-bolt"></i> dari PKPT</small>
                        <?php endif; ?>
                    </div>
                </div>
...
function updateKunjungan() {
    const mg  = document.getElementById('sel-kunjungan-mg').value;
    const bln = document.getElementById('sel-kunjungan-bln').value;
    const thn = document.getElementById('sel-kunjungan-thn').value;
    const val = (mg && bln && thn) ? mg + ' ' + bln + ' ' + thn : '';
    document.getElementById('hid-kunjungan').value = val;
    const prev = document.getElementById('preview-kunjungan');
    prev.innerHTML = val
        ? '<i class="fas fa-calendar-check"></i> <strong style="color:#6366f1">' + val + '</strong>'
        : '';
}
</script>
<?= $this->endSection() ?>